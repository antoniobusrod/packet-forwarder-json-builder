...
/**
 * Build JSON rxpk message for uplink message (UDP packet forwarder gateway)
 *
 * @function
...
/**
 * Build JSON rxpk message for Join Request message (UDP packet forwarder gateway)
 *
 * @function
 * @param {Buffer} devNonce - Join-request nonce buffer
 * @param {Object} scenario - Scenario properties
 * @param {Object} scenario.application - Application scenario properties
 * @param {number} scenario.application.appeui - Application EUI (JoinEUI)
 * @param {Object} scenario.device - Device scenario properties
 * @param {number} scenario.device.deveui - Device EUI
 * @param {Object} scenario.gateway - Gateway scenario properties
 * @param {number} scenario.gateway.frequency - RX central frequency in MHz (unsigned float, Hz precision)
 * @param {string} scenario.gateway.modulation - Modulation identifier "LORA" or "FSK"
 * @param {string} scenario.gateway.datarate - LoRa datarate identifier (eg. SF12BW500)
 ...
 * @returns {Object} UDP packet forwarder gateway JSON rxpk message
 */
exports.joinRequest = function composeJoinRequest (devNonce, scenario) {
  const loraPayload = loraPacket.fromFields({
    MType: 'Join Request',
    AppEUI: Buffer.from(scenario.application.appeui, 'hex'),
    DevEUI: Buffer.from(scenario.device.deveui, 'hex'),
    DevNonce: devNonce
  }).getPHYPayload()
  const rxpkMessage = composeJsonRxpk(loraPayload, scenario)
  return rxpkMessage
